...
int main() {
    ...
    // Test exact indices for param [0..1]
    ...
    // Test lerp between elements
    chunk = compile_expression_to_bytecode("smooth_select(0.25, 10, 20, 30)");
    result = execute_bytecode(chunk, &params);
    ...
    chunk = compile_expression_to_bytecode("smooth_select(0.75, 10, 20, 30)");
    result = execute_bytecode(chunk, &params);
    if (fabs(result - 25.0f) > EPSILON) {
        printf("Test failed (lerp 2). Expected 25.0, got %f\n", result);
        return 1;
    }

    // Out of bounds upper
    chunk = compile_expression_to_bytecode("smooth_select(1.5, 10, 20, 30)");
    result = execute_bytecode(chunk, &params);
    if (fabs(result - 30.0f) > EPSILON) {
    ...
    // smooth_select(0.25, sin(PI/2), max(5, 10), pow(2, 3))
    // 0.25 with 3 items maps to f_index 0.5. Lerps between sin(PI_OVER_2) (1.0) and max(5, 10) (10.0) -> 1.0 + 0.5 * 9.0 = 5.5
    chunk = compile_expression_to_bytecode("smooth_select(0.25, sin(PI_OVER_2), max(5, 10), pow(2, 3))");
    result = execute_bytecode(chunk, &params);

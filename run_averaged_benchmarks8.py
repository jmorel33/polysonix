...
def main():
    ...
    report_5_times = {}
    try:
        with open('doc/PERFORMANCE_REPORT7.md', 'r') as f:
            for line in f:
                if line.startswith('| ') and not line.startswith('| Patch ID'):
                    ...
                    if len(parts) >= 6:
                        try:
                            idx = int(parts[1])
                            # Use the "after" time of report 7 as our new baseline
                            baseline_time = float(parts[4])
                            report_5_times[idx] = baseline_time
                        except ValueError:
                            pass
    except FileNotFoundError:
        print("Could not find doc/PERFORMANCE_REPORT7.md")
        sys.exit(1)
    ...
    out_lines = [
        ...
        "",
        f"This report compares the execution time of the new VM against the PERFORMANCE_REPORT7.md baseline.",
    ...
    out_lines.append(f"* **Average Time per Sample (Report 7):** {avg_baseline:.2f} ns")
    ...
    with open('doc/PERFORMANCE_REPORT8.md', 'w') as f:
        f.write('\n'.join(out_lines))

    print("Report generated and saved to doc/PERFORMANCE_REPORT8.md")

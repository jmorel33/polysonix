...
def main():
            ...
            for line in f:
                if line.startswith('| ') and not line.startswith('| Patch ID'):
                    ...
                    if len(parts) >= 6:
                        try:
                            idx = int(parts[1])
                            # Use the "after" time of report 5 as our new baseline
                            baseline_time = float(parts[4])
                            report_5_times[idx] = baseline_time
                        except ValueError:
                            pass
    except FileNotFoundError:
        print("Could not find doc/PERFORMANCE_REPORT_5.md")
        sys.exit(1)
    ...
    with open('doc/PERFORMANCE_REPORT6.md', 'w') as f:
        f.write('\n'.join(out_lines))

    print("Report generated and saved to doc/PERFORMANCE_REPORT6.md")
